...
class DatabaseList(models.Model):
    ...
    dbname = models.CharField(max_length=30, null=True)  # 数据库名称
    can_search = models.IntegerField(default=0)  # 是否可以用于dbsearch,0为不可以，1是可以
    dbrole = models.IntegerField(default=1) # 0 主库，1 从库
...
class user_collect(models.Model):
    ...
    collect_alias = models.CharField(max_length=100, null=True) # 收藏名
    connect_name = models.CharField(max_length=100, null=True)  # 连接名
    sql_text = models.CharField(max_length=4000, null=True)  # sql文本
