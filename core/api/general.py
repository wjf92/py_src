        ...
        if args == 'connection':
            try:
                ...
                if request.data['permissions_type'] == 'user' or request.data['permissions_type'] == 'own_space':
                    ...
                    for con_instance in DatabaseList.objects.filter(can_search=1):
                        if con_instance:
                            con_name.append(
                                {
                                    'id': con_instance.id,
                ...
                elif request.data['permissions_type'] == 'dml':
                    # dml工单界面获取dblistif
                    con_name = []
                    for con_instance in DatabaseList.objects.filter(dbrole=0):
                        if con_instance:
                            con_name.append(
                                {
                                    'id': con_instance.id,
