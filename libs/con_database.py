    ...
    def search(self, sql=None):
        ...
        with self.con.cursor(cursor=pymysql.cursors.DictCursor) as cursor:
            ...
            for field in cursor.description:
                if id == 0:
                    data_dict.append({'title': field[0], "key": field[0], "fixed": "left", "sortable": "true", "width": 150})
                    id += 1
                else:
                    data_dict.append({'title': field[0], "key": field[0], "width": 200, "sortable": "true"})
            len = cursor.rowcount
            elapsed_time = (end_time - start_time).total_seconds()
        return {'data': result, 'title': data_dict, 'len': len, 'elapsed_time': elapsed_time}
